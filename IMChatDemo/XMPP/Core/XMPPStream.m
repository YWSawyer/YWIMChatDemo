...
#if DEBUG
  static const int xmppLogLevel = XMPP_LOG_LEVEL_INFO | XMPP_LOG_FLAG_SEND_RECV | XMPP_LOG_FLAG_TRACE | XMPP_LOG_FLAG_RECV_PRE;
#else
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	XMPPLogRecvPre(@"DATA RECV: %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
...
- (void)xmppParser:(XMPPParser *)sender didReadRoot:(NSXMLElement *)root
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"ROOT RECV: %@", [root compactXMLString]);
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	XMPPLogTrace();
	XMPPLogRecvPost(@"ELEMENT RECV: %@", [element compactXMLString]);
